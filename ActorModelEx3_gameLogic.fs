...
let getExit direction exits=
    ...
    | LockedExit (_) -> Failure "Ther is a locked door in that direction"
...
let applyUpdate updateFunc worldState=
    ...
    | Success newState -> describeCurrentRoom newState |> displayResult
                          newState
    | Failure message -> printfn "\n\n%s\n" message  
                         worldState
            ...
            async{
                try
                    ...
                    if eventMsg="Stop" then return ()
                            ...
                            let direction=match eventMsg with
                                          | "north" -> north 
                                          | "south" -> south
                                          | "east" -> east
                                          | "west" -> west
                                          | _ -> north
                            gameEngine.ApplyUpdate (move direction)
                            do! innerLoop state
                with
                | :? System.TimeoutException ->
                        ...
                        printfn "Wandering %s.." (fst direction)  
                        gameEngine.ApplyUpdate (move (snd direction))
                        do! innerLoop state
            }
        innerLoop 0
    )
